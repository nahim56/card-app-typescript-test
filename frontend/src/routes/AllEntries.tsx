      ...
      {entries.map((entry: Entry, index: number) => {
        return (
          <div
            id={entry.id}
            key={index}
            className="bg-gray-300 shadow-md shadow-gray-500 m-3 p-4 rounded flex flex-col dark:bg-slate-800  justify-between"
          >
            <h1 className="font-bold text-sm md:text-lg dark:text-white break-words">{entry.title}</h1>
            <p className="text-center text-lg font-light md:mt-2 md:mb-4 mt-1 mb-3  dark:text-white break-words">
              {entry.description}
            </p>
